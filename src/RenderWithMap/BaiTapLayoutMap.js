...
export default class BaiTapLayoutMap extends Component {
    ...
    renderFilm = () => {
        return this.dataPhim.map((product, index) => {
            return <div className="col-4 mt-5" key={index}>
                <div className="card text-left">
                    <div style={{ background: `url("${product.hinhAnh}") no-repeat center`, backgroundSize: "cover" }}>
                        <img style={{ opacity: "0" }} className="card-img-top" src={product.hinhAnh} height={300} alt="123" />
                    </div>
                    <div className="card-body">
                        <h4 style={{ height: 50 }} className="card-title">
                            {product.tenPhim.length > 20 ? <span>{product.tenPhim.substr(0, 20)}...</span> : <span>{product.tenPhim}</span>}
                        </h4>
                        <p style={{ height: 70 }} className="card-text">
                            {product.moTa.length > 100 ? <span> {product.moTa.substr(0, 100)}...</span> : <span> {product.moTa}</span>}
                        </p>
                    </div>
                </div>
